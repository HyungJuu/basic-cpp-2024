...
- 클래스(class)
  ...
  &rarr; 멤버변수는 public() 함수를 붙이고 멤버함수는 private()함수를 붙인다.
...
- 복사 생성자 : 객체를 복사할 때 호출되는 생성자  
				&rarr; 따라서 복사할 객체를 입력으로 가져야 하는데 이때 원본 객체의 참조(&)를 입력으로 받아야 함  
				&rarr; 복사생성자를 객체로 받으면 받아들이는 과정에서 복사가 무한반복
``` C++
SoSimple sim2(sim1); // 복사생성자 호출
```

- 디폴트 생성자 : 기본적으로 제공되는 생성자. 생성자를 명시적으로 만들었을 때는 소멸(제공X).  
				&rarr; 디폴트 생성자가 필요할 경우에는 명시적으로 작성
...
- explicit : 복사 생성자의 묵시적 호출을 허용X  
		&rarr; 대입 연산자를 이용한 객체의 생성 및 초기화 불가능
...
## 8일차
// p.326 ~ 382
- 부모 객체 포인터는 자식 객체 포인터를 가리킬 수 있다. (부모 &rarr; 자식) ( 부모 &larr; 자식 X )
